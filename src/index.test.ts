...
jest.mock('./config', () => ({
  getConfig: jest.fn(() => ({
    ...
    defaultDomain: 'local',
    logLevel: 'info',
    reverseProxyIps: ['xxx.xxx.xxx.xxx'],
  })),
}));
...
describe('buildDesiredRecords (logic extracted)', () => {
  ...
  const reverseProxyIps = ['xxx.xxx.xxx.xxx', 'xxx.xxx.xxx.xxx'];

  describe('IP address filtering', () => {
    it('should skip hosts that are IP addresses', () => {
      const routers: TraefikRouterInfo[] = [
        { name: 'router-1', hosts: ['xxx.xxx.xxx.xxx'] },
        { name: 'router-2', hosts: ['xxx.xxx.xxx.xxx'] },
      ...
      expect(result).toContainEqual({ domain: 'example.com', ip: 'xxx.xxx.xxx.xxx' });
      expect(result).toContainEqual({ domain: 'example.com', ip: 'xxx.xxx.xxx.xxx' });
    });
  ...
  describe('multiple reverse proxy IPs', () => {
    it('should create records for each reverse proxy IP', () => {
      ...
      const ips = ['xxx.xxx.xxx.xxx', 'xxx.xxx.xxx.xxx', 'xxx.xxx.xxx.xxx'];
  ...
  describe('edge cases', () => {
    ...
    it('should handle all hosts being filtered out', () => {
      const routers: TraefikRouterInfo[] = [
        { name: 'router-1', hosts: ['xxx.xxx.xxx.xxx', 'localhost', '^regex$'] },
      ];
    ...
    it('should handle single reverse proxy IP', () => {
      ...
      const result = buildDesiredRecords(routers, ['xxx.xxx.xxx.xxx']);

      expect(result).toHaveLength(1);
      expect(result[0]).toEqual({ domain: 'example.com', ip: 'xxx.xxx.xxx.xxx' });
    });
...
describe('Sync flow integration (with mocked services)', () => {
  ...
  it('should handle remove and update operations correctly', async () => {
    ...
    const desiredRecords: DnsRecord[] = [
      { domain: 'new.example.com', ip: 'xxx.xxx.xxx.xxx' },
    ];
