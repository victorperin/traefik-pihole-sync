...
describe('generateDiff', () => {
  ...
  const currentRecords: DnsRecord[] = [
    ...
    { domain: 'old.example.com', ip: 'xxx.xxx.xxx.xxx' },
    { domain: 'unchanged.example.com', ip: 'xxx.xxx.xxx.xxx' },
  ];
  
  const desiredRecords: DnsRecord[] = [
    { domain: 'example.com', ip: 'xxx.xxx.xxx.xxx' }, // unchanged
    { domain: 'new.example.com', ip: 'xxx.xxx.xxx.xxx' }, // new
    { domain: 'changed.example.com', ip: 'xxx.xxx.xxx.xxx' }, // changed (was different IP)
  ];
  ...
  describe('records to add', () => {
    it('should return records that exist in desired but not in current as toAdd', () => {
      ...
      expect(diff.toAdd).toContainEqual({ domain: 'example.com', ip: 'xxx.xxx.xxx.xxx' });
      expect(diff.toAdd).toContainEqual({ domain: 'new.example.com', ip: 'xxx.xxx.xxx.xxx' });
      expect(diff.toAdd).toContainEqual({ domain: 'changed.example.com', ip: 'xxx.xxx.xxx.xxx' });
    });
  ...
  describe('records to remove', () => {
    it('should return records that exist in current but not in desired as toRemove', () => {
      ...
      expect(diff.toRemove).toContainEqual({ domain: 'old.example.com', ip: 'xxx.xxx.xxx.xxx' });
    });
  ...
  describe('records to change', () => {
    it('should return records with same domain but different IP as toChange', () => {
      ...
      expect(diff.toChange).toContainEqual({ domain: 'changed.example.com', ip: 'xxx.xxx.xxx.xxx' });
    });

    it('should not include unchanged records in toChange', () => {
      const diff = generateDiff(currentRecords, desiredRecords);

      expect(diff.toChange).not.toContainEqual({ domain: 'example.com', ip: 'xxx.xxx.xxx.xxx' });
    });
  });
  ...
  describe('edge cases', () => {
    ...
    it('should handle duplicate domains in current records (uses last one)', () => {
      const currentWithDuplicates: DnsRecord[] = [
        { domain: 'example.com', ip: 'xxx.xxx.xxx.xxx' },
        { domain: 'example.com', ip: 'xxx.xxx.xxx.xxx' }, // duplicate
      ];
    ...
    it('should handle single record add, remove, and change simultaneously', () => {
      const currentSingle: DnsRecord[] = [
        { domain: 'old.com', ip: 'xxx.xxx.xxx.xxx' },
      ];
      
      const desiredSingle: DnsRecord[] = [
        { domain: 'new.com', ip: 'xxx.xxx.xxx.xxx' },
      ];
      
      const diff = generateDiff(currentSingle, desiredSingle);

      expect(diff.toAdd).toContainEqual({ domain: 'new.com', ip: 'xxx.xxx.xxx.xxx' });
      expect(diff.toRemove).toContainEqual({ domain: 'old.com', ip: 'xxx.xxx.xxx.xxx' });
      expect(diff.toChange).toHaveLength(0);
    });
  });
});
...
describe('PiHoleService', () => {
  ...
  describe('addDnsRecord', () => {
    it('should make correct PUT request to add DNS record', async () => {
      ...
      await service.addDnsRecord('example.com', 'xxx.xxx.xxx.xxx');

      expect(mockedAxios.put).toHaveBeenCalledTimes(1);
      expect(mockedAxios.put).toHaveBeenCalledWith(
        'http://pihole:80/api/config/dns/hosts/xxx.xxx.xxx.xxx example.com',
        {
    ...
    it('should not throw when axios request fails', async () => {
      ...
      // Should not throw
      await expect(service.addDnsRecord('example.com', 'xxx.xxx.xxx.xxx')).resolves.not.toThrow();
    });
  });
  ...
  describe('removeDnsRecord', () => {
    it('should make correct DELETE request to remove DNS record', async () => {
      ...
      await service.removeDnsRecord('example.com', 'xxx.xxx.xxx.xxx');
    ...
    it('should not throw when axios request fails', async () => {
      ...
      // Should not throw
      await expect(service.removeDnsRecord('example.com', 'xxx.xxx.xxx.xxx')).resolves.not.toThrow();
    });
  });
  ...
  describe('listDnsRecords', () => {
    it('should return parsed DNS records from Pi-hole API response', async () => {
      const mockResponse = {
        data: {
          config: {
            dns: {
              hosts: ['xxx.xxx.xxx.xxx example.com', 'xxx.xxx.xxx.xxx old.example.com'],
            },
          },
        },
      };
      ...
      expect(result).toEqual(['xxx.xxx.xxx.xxx example.com', 'xxx.xxx.xxx.xxx old.example.com']);
    });
    ...
    it('should fallback to response data when config structure is missing', async () => {
      mockedAxios.get.mockResolvedValueOnce({ data: ['xxx.xxx.xxx.xxx example.com'] });

      const result = await service.listDnsRecords();

      expect(result).toEqual(['xxx.xxx.xxx.xxx example.com']);
    });
  });
    ...
    it('should parse raw DNS records into DnsRecord objects', async () => {
      mockedAxios.get.mockResolvedValueOnce({
        data: {
          config: {
            dns: {
              hosts: ['xxx.xxx.xxx.xxx example.com', 'xxx.xxx.xxx.xxx sub.domain.com'],
            },
          },
        },
      });
      ...
      expect(result).toEqual([
        { domain: 'example.com', ip: 'xxx.xxx.xxx.xxx' },
        { domain: 'sub.domain.com', ip: 'xxx.xxx.xxx.xxx' },
      ]);
    });
    ...
    it('should handle IP and domain with spaces in domain name', async () => {
      mockedAxios.get.mockResolvedValueOnce({
        data: {
          config: {
            dns: {
              hosts: ['xxx.xxx.xxx.xxx sub domain with spaces.com'],
            },
          },
        },
      });
      ...
      expect(result).toEqual([
        { domain: 'sub domain with spaces.com', ip: 'xxx.xxx.xxx.xxx' },
      ]);
    });
    ...
    it('should skip malformed entries that do not have IP and domain', async () => {
      mockedAxios.get.mockResolvedValueOnce({
        data: {
          config: {
            dns: {
              hosts: ['xxx.xxx.xxx.xxx example.com', 'invalid-entry', 'xxx.xxx.xxx.xxx'],
            },
          },
        },
      });
